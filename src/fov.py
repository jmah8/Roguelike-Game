...
def new_fov(game):
    ...
    """
    new_fov = [[0 for x in range(0, game.map_data.map_width)] for y in range(game.map_data.map_height)]
    return new_fov
...
def ray_casting(game, map_array, fov):
    ...
    for a in range(0, RAYS + 1, STEP):
        ...
        for b in range(KNIGHT_FOV):
            ...
            if x < 0 or y < 0 or x > game.map_data.map_width - 1 or y > game.map_data.map_height - 1:
                break
...
def draw_seen(game, map_array, fov):
    ...
    Args:
        ...
        fov: fov array telling which tile is seen
    """
    for y in range(0, game.map_data.map_height):
        for x in range(0, game.map_data.map_width):
            tile = map_array[y][x]
